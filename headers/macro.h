...
#define PREPARE_PAYMENT_SIMPLE(buf_out_master, drops_amount_raw, to_address, dest_tag_raw, src_tag_raw, sizeout)\
    {\
        uint8_t* buf_out = buf_out_master;\
        uint8_t acc[20];\
        ...
        uint32_t remaining_size = PREPARE_PAYMENT_SIMPLE_SIZE - (buf_out - buf_out_master);\
        int64_t edlen = etxn_details((uint32_t)buf_out, remaining_size);                    /* emitdet | size 1?? */ \
        buf_out += edlen;\
        sizeout = (buf_out - buf_out_master);\
        int64_t fee = etxn_fee_base(buf_out_master, sizeout);\
        _06_08_ENCODE_DROPS_FEE            (fee_ptr, fee                            );                               \
...
#endif
#define PREPARE_PAYMENT_SIMPLE_TRUSTLINE(buf_out_master, tlamt, to_address, dest_tag_raw, src_tag_raw, sizeout)\
    {\
        uint8_t* buf_out = buf_out_master;\
        uint8_t acc[20];\
        uint32_t dest_tag = (dest_tag_raw);\
        ...
        _08_01_ENCODE_ACCOUNT_SRC          (buf_out, acc                            );      /* account | size  22 */ \
        _08_03_ENCODE_ACCOUNT_DST          (buf_out, to_address                     );      /* account | size  22 */ \
        uint32_t remaining_size = PREPARE_PAYMENT_SIMPLE_TRUSTLINE_SIZE - (buf_out - buf_out_master);\
        int64_t edlen = etxn_details((uint32_t)buf_out, remaining_size);                    /* emitdet | size 1?? */  \
        buf_out += edlen;\
        sizeout = (buf_out - buf_out_master);\
        int64_t fee = etxn_fee_base(buf_out_master, sizeout);\
